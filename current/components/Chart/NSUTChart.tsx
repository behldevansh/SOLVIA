...
// Currency formatter
function formatPriceINR(amount: any) {
...
export default function NSUTCHART({ formData }: any) {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const powerResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/nsut/range`,
          {
            method: "POST",
            body: JSON.stringify({
        ...
        const dustResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/nsut/dust`,
          {
            method: "POST",
            body: JSON.stringify({
              start_date: format(formData.from, "yyyy-MM-dd"),
